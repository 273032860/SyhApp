...
export function SpotLight({
  ...
  useEffect(() => {
    if (lightRef.current) {
      ...
      lightRef.current.power = power ;  //intensity * Math.PI
      lightRef.current.intensity = power / (Math.PI*36);
    ...
    colorL, power, distance, decay, angle, penumbra, position, target,
    castShadow, visible,
    shadowMapSize, shadowRadius, shadowBias, shadowNormalBias, shadowNear, shadowFar
  ]);

  useHelper(lightRef, isHelper && SpotLightHelper);
  return (
    <>
      {debug && <SpotLightBuilder levaname={levaname} settings={settings} onChange={setSettings} />}
      <spotLight
        name={levaname}
        ref={lightRef}
...
const SpotLightBuilder = ({ levaname, settings, onChange }) => {
  ...
  const controls = useControls(levaname, {
    ...
    '光束形状':folder({
      angle: {
        value: settings.angle || Math.PI / 6,
        min: 0,
        max: Math.PI / 2,
        step: 0.01,
        label: "尺寸"
    ...
    "Export Settings": button(() => {
      const exportedSettings = {
        ...
        angle: curControls.current.angle,
        penumbra: curControls.current.penumbra,
